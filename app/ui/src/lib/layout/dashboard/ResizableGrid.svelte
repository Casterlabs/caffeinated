<script lang="ts">
	...
	interface Props {
		contents: Record<string, [String | Component<any, any, any>, object?]>;
		onupdate?: (layout: Layout) => void;
		maxSize?: number;
	}

	let { contents = $bindable(), onupdate, maxSize = 3 }: Props = $props();
	...
	let vlayout = $state([0.25, 0.33]); // last item is implied.
	let hlayout = $state([0.33, 0.45]); // last item is implied.
	let width = $derived(vlayout.length + 1);
	let height = $derived(hlayout.length + 1);

	function onLayoutUpdated(remount = true) {
		// Notify of our updated layout.
		onupdate?.({ v: [...vlayout], h: [...hlayout] });
	...
	export function updateLayout(newLayout: Layout) {
		vlayout = newLayout.v;
		hlayout = newLayout.h;
		tick().then(() => onLayoutUpdated(false));
	}
	...
	function onMouseMove(e: MouseEvent) {
		...
		const { x: containerOffsetX, y: containerOffsetY, width: containerWidth, height: containerHeight } = container.getBoundingClientRect();
		const { clientX: mouseX, clientY: mouseY } = e;

		// We add 3.5 to X to keep the sizer bar centered on the mouse.
		const mousePosition = [(mouseX + 3.5 - containerOffsetX) / containerWidth, (mouseY - containerOffsetY) / containerHeight];
...
<div class="relative w-full h-full">
	<div class="hlayout" bind:this={container}>
		{#each Array(height) as _, hindex}
					...
					{#each Array(width) as _, vindex}
						...
						<div class="vslot-area" style="--size: {vSize};" data-position={location}>
							{#if typeof content == 'string'}
								{@html content}
							{:else}
								{@const TypedComponent = content as Component}
						...
						{#if !isVLast}
							...
							<div class="vsizer-bar oversize" class:cursor-col-resize={!$isResizingLocked} onmousedown={(e) => onMouseDown(e, true, vindex)}>
								<div class="inner"></div>
							</div>
						{/if}
					{/each}
			...
			{#if !isHLast}
				...
				<!-- svelte-ignore a11y_no_static_element_interactions -->
				<div class="hsizer-bar" class:cursor-row-resize={!$isResizingLocked} onmousedown={(e) => onMouseDown(e, false, hindex)}>
					<div class="inner"></div>
				</div>
			{/if}
		{/each}
	...
	<div class="fixed top-0 right-4 h-10 z-50 -translate-y-[2.15rem] hover:translate-y-0 px-2 pt-1 transition-all rounded-b-md ring-1 ring-base-7 bg-base-5 text-base-12 opacity-90">
		...
		<Input
			...
			disabled={$isResizingLocked}
			onchange={(e) => {
				const newHeight = (e.target as HTMLInputElement).valueAsNumber;
