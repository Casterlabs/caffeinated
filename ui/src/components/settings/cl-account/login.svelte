<script>
    ...
    async function submit() {
        ...
        try {
            account = await Account.login(emailInput, passwordInput);
            currentScreen = "ME";
        } catch (e) {
            console.error(e);
...
<Layout>
    <h1 class="title has-text-centered">
        <LocalizedText key="casterlabs_account.login_to_casterlabs" />
    </h1>
    <br />
    ...
    <div class="field is-grouped">
        ...
        <div class="control">
            <button class="button is-link {isLoading ? 'is-loading' : ''}" on:click={submit}>
                <LocalizedText key="casterlabs_account.login" />
            </button>
        </div>
    </div>
    ...
    <div class="has-text-centered">
        <a on:click={() => (currentScreen = "REGISTER")}>
            <LocalizedText key="casterlabs_account.need_an_account" />
        </a>
        <br />
        <a on:click={() => (currentScreen = "RESET_PASSWORD")}>
            <LocalizedText key="casterlabs_account.forgot_your_password" />
        </a>
    </div>
</Layout>
